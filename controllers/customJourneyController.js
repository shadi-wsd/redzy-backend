...
const createCustomJourneys = async (req, res, next) => {
    const {journeyName, breakPoints, maxStagesNumber, productValPerc, pointsCommission} = req.body
    ...
    const customJourney = await createCustomJourney({journeyName, breakPoints, maxStagesNumber: stagesNumber, productValue, pointsCommission})
    if(!customJourney){
...
const editCustomJourneys = async (req, res, next) => {
    const {customJourneyId, journeyName, breakPoints, maxStagesNumber, productValPerc, pointsCommission} = req.body
    if (!customJourneyId){
        return next(new ErrorHandler(FieldsMandotry, 400))
    }
    ...
    const updateData = {journeyName, breakPoints, maxStagesNumber, productValue: productValPerc, pointsCommission}
